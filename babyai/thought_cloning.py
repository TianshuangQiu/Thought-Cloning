...
class OnlineLearning(object):
    def __init__(
        ...
        self.log_list = {'return_per_episode':[],
        ...
        # Define actor-critic model
        #self.acmodel = utils.load_model(args.model, raise_not_found=False)
        self.rl_algo = PPOAlgo([self.env],self.acmodel,args.model)
        for i in range(20):
            log = self.rl_algo.update_parameters()
            self.log_list['return_per_episode'].append(log['return_per_episode'])
            self.log_list['reshaped_return_per_episode'].append(log['reshaped_return_per_episode'])
            self.log_list['num_frames_per_episode'].append(log['num_frames_per_episode'])
            self.log_list['num_frames'].append(log['num_frames'])
            self.log_list['episodes_done'].append(log['episodes_done'])
            self.log_list['entropy'].append(log['entropy'])
            self.log_list['value'].append(log['value'])
            self.log_list['policy_loss'].append(log['policy_loss'])
            self.log_list['value_loss'].append(log['value_loss'])
            self.log_list['grad_norm'].append(log['grad_norm'])
            self.log_list['loss'].append(log['loss'])
            print(i)
        with open(os.getcwd()+'/ppo_logs/ppo_log.json','w') as f:
...
class OnlineLanguageLearning(object):
    def __init__(
        self,
        args,
    ):
